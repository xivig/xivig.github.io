...
var OverlayScrollbarsGlobal = function(t) {
  ...
  const createOverflowUpdateSegment = (t, n) => {
    ...
    const getMeasuredScrollCoordinates = (t, n) => {
      ...
      const i = addEventListener(f, D, (t => {
        const n = getElementScroll(_);
        if (t.isTrusted && n.x === c.x && n.y === c.y) {
          stopPropagation(t);
        }
      }), {
        I: true,
      ...
      const m = {
        x: r(h.x) >= r(S.x) ? g.x : b.x,
        y: r(h.y) >= r(S.y) ? g.y : b.y
      };
      scrollElementTo(_, c);
      a((() => i()));
      return {
    ...
    return ({Dt: o, Zt: s, fn: r, It: a}, {dn: u}) => {
      ...
      const dt = u || y || S || at || it || st || ct || X || j || G || _ && h;
      const [pt] = o("update.flowDirectionStyles");
      const [vt, gt] = L(pt ? pt(l) : getFlowDirectionStyles(), a);
      const ht = y || w || gt || ft || a;
      const [bt, yt] = ht ? P(getMeasuredScrollCoordinates(vt, !!pt), a) : U();
      let wt = getViewportOverflowStyle(ut, F);
      setMeasuringMode(false);
      if (dt) {
        setViewportOverflowStyle(wt);
        wt = getElementOverflowStyle(l, ut);
        if (D && H) {
          H(wt, rt, lt);
          setStyles(l, D(wt));
        }
