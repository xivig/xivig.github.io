...
var OverlayScrollbarsGlobal = function(r) {
  ...
  var pn = function createOverflowUpdateSegment(r, a) {
    ...
    var A = function getMeasuredScrollCoordinates(r, e) {
      ...
      x(true);
      var i = he(d);
      var v = se(s, rr, (function(r) {
        var e = he(d);
        if (r.isTrusted && e.x === i.x && e.y === i.y) {
          de(r);
        }
      }), {
        H: true,
      ...
      var O = {
        x: o(h.x) >= o(m.x) ? g.x : b.x,
        y: o(h.y) >= o(m.y) ? g.y : b.y
      };
      ge(d, i);
      f((function() {
        return v();
    ...
    return function(e, n) {
      ...
      var Lr = K && rr && (kr.x || kr.y) || K && kr.x && !kr.y || rr && kr.y && !kr.x;
      var Vr = s || w || C || zr || Dr || Sr || Cr || J || X || er || y && b;
      var Rr = t("update.flowDirectionStyles"), Fr = Rr[0];
      var Nr = Q(Fr ? Fr(c) : E(), l), jr = Nr[0], Ur = Nr[1];
      var qr = w || O || Ur || Ir || l;
      var Br = qr ? ir(A(jr, !!Fr), l) : or(), Yr = Br[0], Wr = Br[1];
      var Xr = D(kr, $);
      x(false);
      if (Vr) {
        H(Xr);
