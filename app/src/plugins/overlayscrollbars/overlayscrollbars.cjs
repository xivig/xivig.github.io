...
const createStructureSetupElements = t => {
  ...
  const X = {
    ...
    yt: t => hasAttrClass(A, F, t),
    wt: (t, n) => addRemoveAttrClass(A, F, t, n),
    St: () => addRemoveAttrClass(P, F, G, true)
  };
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  return ({Dt: n, Zt: o, fn: c, It: l}, {dn: u}) => {
    ...
    const {X: x, Y: H, W: E} = m || {};
    const [T, B] = getShowNativeOverlaidScrollbars(n, e);
    ...
    const dt = u || y || S || at || it || st || ct || X || B || J || _ && h;
    const [pt] = n("update.flowDirectionStyles");
    const [vt, gt] = L(pt ? pt(a) : getFlowDirectionStyles(), l);
    const ht = y || w || gt || ft || l;
    const [bt, yt] = ht ? P(getMeasuredScrollCoordinates(vt, !!pt), l) : U();
